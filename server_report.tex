...
% Allow LaTeX a bit more wiggle room before declaring overfull hboxes;
% needed because paragraphs include long un-hyphenable \texttt{} fragments
% (filenames, dotted env vars, URLs) that would otherwise overflow.
\setlength{\emergencystretch}{3em}
\hbadness=99999  % silence underfull-hbox warnings
\hfuzz=2pt       % tolerate <= 2pt overflow without emitting a warning
...
\begin{center}
  \begin{tikzpicture}
    ...
    ] {Go \(\cdot\) Python \(\cdot\) NeMo FastConformer \(\cdot\) Docker \(\cdot\) Caddy};
  \end{tikzpicture}
\end{center}
